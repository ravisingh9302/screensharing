...
const RoomPage = () => {
  ...
  const handleCallUser = useCallback(async () => {
    ...
    console.log("screen", display.getTracks())
    let abcd = new MediaStream()
    console.log("new media ", abcd.getTracks())
    let strm = stream.getTracks()
    let strm2 = display.getTracks()
    abcd.addTrack(strm[0])
    abcd.addTrack(strm[1])
    abcd.addTrack(strm2[0])
    setMyStream(abcd);
    const offer = await peer.getOffer();
    socket.emit("user:call", { to: remoteSocketId, offer });
    document.getElementById('myStream_player').srcObject = stream
    document.getElementById('video-screen').srcObject = display
  ...
  useEffect(() => {
    peer.peer.addEventListener("track", async (ev) => {
      console.log("all track", ev.streams)
      // const [remoteStream] = ev.streams;
      const remoteStream = ev.streams;
      console.log("remote stream", remoteStream[0].getTracks())
      let allstrm = remoteStream[0].getTracks()
      console.log("GOT TRACKS!!");
      if (allstrm.length > 2) {
        let strem1 = new MediaStream()
        let strem2 = new MediaStream()
        strem1.addTrack(allstrm[0])
        strem1.addTrack(allstrm[1])
        strem2.addTrack(allstrm[2])
        setRemoteStream(remoteStream);
        setRemoteStream(remoteStream);
        setCallbtn(false)
        document.getElementById('video-screen').srcObject = strem2
        document.getElementById('myStream_player').srcObject = strem1
        return
      }
      else {
        // let stream3 =new MediaStream()
        document.getElementById('video-screen').srcObject = remoteStream[0]
        setCallbtn(false)
        return
      }
    ...
    <div className="bg-black h-screen p-2">
      <div id="chat-page" className="h-full relative p-2">
        ...
        <div id="video-remote" className="absolute top-0 bottom-0 left-0 right-0 overflow-auto  bg-slate-300 ">
          ...
          <video style={{ width: "100%", height: "100%", objectFit: "cover" }} id="video-screen" autoPlay playsInline></video>
          {/* )} */}
        </div>
